...
%Image Source: http://brevia.hcura.org/wp-content/uploads/2013/08/stem-cells.jpg
%Reading image of cells:
IM=imread('stem-cells.jpg');
...
%Cells are labeled red
%Showing only the red components of the image
figure(2);
imshow(R);
title('Red Channel of Image');
...
%Trying to find nuclei for Thresholded and Low passed filtered image
...
title('Finding nuclei in Low Pass filtered Image');
[cent3,rd3]=imfindcircles(R,[10 30]);
figure(2);
viscircles(cent3,rd3,'EdgeColor','r');
title('Finding nuclei in Red only channel of Image');
%% Testing Ability to Find Nuclei in High Pass filtered Image
...
title('Nuclei found In High Pass filtered Image');
