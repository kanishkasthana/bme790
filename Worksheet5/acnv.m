...
% definition of function 'f(t)'
   f=zeros(1,length(t));
   f(t>=0 & t<1)=ones(1,length(find(t>=0 & t<1)));
   f(t>=1 & t<=2)=exp(-1.*t(t>=1 & t<=2) +1);
%  f = 0.1*(t.^2);
%  f = 5*ones(1, length(t)); 
%  f = t;
...
go=zeros(1,length(t1));
go(t1>=1 & t1<=4)=exp(-1.*t1(t1>=1 & t1<=4) +1);
% go = .1*(t1.^3);
% go = 5*cos(2*pi*t1);
...
% convolve: note the multiplation by the sampling interval
  c = s_int * conv(f, go);
%KANISHK NOTE: By increasing the sampling rate or decreasing dt for the
%same continuous function when we calculate the area under the x[n] we have
%to mulitiply by dt to make sure the area is not dependent on sampling
%rate. This is confusing I have to get into the details of this.
% Animation %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
